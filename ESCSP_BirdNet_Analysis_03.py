...
Base_dir="/media/tracy/ESCSPA03/2024_Total_Eclipse_Data_Terk/eclipse_recordings"
sites=["ESID#516",
         "ESID#517"
         ]
...
#Find all of the wave files in the folder
    ...
    input_files=glob.glob(os.path.join(Base_dir,site+"_Non-Eclipse-Day","*."+"WAV"))
...
# Save DataFrame to CSV
    output_file=os.path.join(Base_dir, f"{site}_non_eclipse_species_list.csv")
...
#Find all of the wave files in the folder
    ...
    input_files=glob.glob(os.path.join(Base_dir,site+"_Total_","*."+"WAV"))
    input_files=natsorted(input_files)
    for input_file in input_files:
...
# Preview the DataFrame structure
    print(df.head())
# Save DataFrame to CSV
    output_file=os.path.join(Base_dir, f"{site}_eclipse_species_list.csv")
